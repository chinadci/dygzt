		...
		for(ResearchTableRow rowmodel:model.getRowList()){
			...
			for(ResearchTableCell value:rowmodel.getValue()){
				if(value.getValue().equals("0")){
					cell= new ResearchTableCellVO("",1,1,null,null,null);
				}else if(value.getCondition()==null){
					cell= new ResearchTableCellVO(value.getValue(),1,1,null,null,null);
				}else{
					cell= new ResearchTableCellVO(value.getValue(),1,1,value.getBase().replace("%", "%25"),value.getCondition(),value.getFydm());
				}
				valueList.get(level).add(cell);
			}
		}
