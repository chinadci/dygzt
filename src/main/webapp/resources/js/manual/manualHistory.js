$(function(){
	...
	//查看原因
	$(document).on("click",".cpt_menu_reason",function(){
		alertEmpty($(this).data("reason"),"查看原因");
	});
...
//		var undobtn = $(this);
//		$.ajax({
//		url : "manualTableUndo.aj",
//		type : "post",
//		dataType:'json',
//		data : {},
//		success : function(data) {
//			if(data.success==true){
//				undobtn.parent().html(data.text);
//			}else{
...
//	});
	...
	$(document).on("click",".cpt_menu_del",function(){
		...
		td.html(waitImg);
		$.ajax({
			url : "manualHistoryDelete.aj",
			type : "post",
			dataType:'json',
			data : {
				researchid:$(this).data("bbid")
			},
			success : function(data) {
				if(data.success==true){
					td.html(data.text);
...
function initTable(){
	...
	$("#table").DataTable({
		...
		"language": {
		 ...
		 "bServerSide" : true,
		 "sAjaxSource": "manualHistoryList.aj?ksrq="+$("#ksrq").val()+"&jsrq="+$("#jsrq").val(),
		 "aoColumns": [
		 	{ "mDataProp": "dyrq" },
		 	{ "mDataProp": "dymd" },
		 	{ "mDataProp": "ksrq" },
